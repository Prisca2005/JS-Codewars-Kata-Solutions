...
function sumDigNthTerm(initVal, patternL, nthTerm) {
    let term = initVal; // Start with the initial value
    let patternLength = patternL.length;
    
    // Compute the nth term
    for (let i = 0; i < nthTerm - 1; i++) { // Since initVal is the first term, we iterate nthTerm - 1 times
        term += patternL[i % patternLength]; // Cycle through pattern list
    }
    
    // Compute sum of digits of the nth term
    return term.toString().split('').reduce((sum, digit) => sum + parseInt(digit), 0);
}
